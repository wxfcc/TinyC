...
void FunctionX64::beginBuild() {
    ...
    emit(0x48, 0x81, 0xec); emitValue((MAX_LOCAL_COUNT -1)* 8); // sub rsp, MAX_LOCAL_COUNT * 8, there should keep rsp align 16 bytes for some instructions, like: movaps XMMWORD PTR [rsp+0x10], xmm1
}
//call from JITEngine
...
void FunctionX64::pop(int n) {
    emit(0x48, 0x81, 0xc4); emitValue(n * 8); // add rsp, n * 8
}
void FunctionX64::dup() {
    emit(0xff, 0x34, 0x24); // push qword ptr [rsp]
}

void FunctionX64::doArithmeticOp(TokenID opType) {
    emit(0x48, 0x8b, 0x44, 0x24, 0x08); // mov rax, qword ptr [rsp+8]
    switch (opType) {
    case TID_OP_ADD:
        emit(0x48, 0x03, 0x04, 0x24); // add rax, qword ptr [rsp]
        break;
    case TID_OP_SUB:
        emit(0x48, 0x2b, 0x04, 0x24); // sub rax, qword ptr [rsp]
        break;
    case TID_OP_MUL:
        emit(0x48, 0x0f, 0xaf, 0x04, 0x24); // imul rax, qword ptr [rsp]
        break;
    case TID_OP_DIV:
    case TID_OP_MOD:
        emit(0x48, 0x31, 0xd2); // xor rdx, rdx
        emit(0x48, 0xf7, 0x3c, 0x24); // idiv qword ptr [rsp]
    ...
    emit(0x48, 0x89, 0x44, 0x24, 0x08); // mov qword ptr [rsp+8], rax
    emit(0x48, 0x83, 0xc4, 0x08); // add rsp, 8
}
void FunctionX64::cmp(TokenID cmpType) {
    Label label_1, label_0, label_end;
    emit(0x48, 0x8b, 0x44, 0x24, 0x08); // mov rax, qword ptr [rsp+8] 
    emit(0x48, 0x8b, 0x14, 0x24); // mov rdx, qword ptr[rsp]
    emit(0x48, 0x83, 0xc4, 0x08); //emitValue((char)8);// add rsp, 8
...
int FunctionX64::beginCall() {
    m_beginCall = 1;
    if (m_paramCount > 0) {
        emit(0x48, 0x89, 0x4d, 0x10);//          mov    QWORD PTR[rsp + 0x08], rcx
    }
    if (m_paramCount > 1) {
        emit(0x48, 0x89, 0x55, 0x18);//          mov    QWORD PTR[rsp + 0x10], rdx
    }
    return 0;
}
/*
...
void FunctionX64::endCall(const string& funcName, int callID, int paramCount) {
    ...
    if (offset > max || offset < min) {
        ...
        emit(0xe9); emitValue(next);         // jmp rip+offset32, this instruction should save in a special section?
